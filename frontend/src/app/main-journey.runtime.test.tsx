 ...
 *
 * The main user journey is now covered by real browser tests at:
 *   ../../e2e/main-journey.spec.ts
 *
 * These runtime tests share a single JSDOM document, which breaks
 * when the form unmounts and a new page renders — JSDOM's browsing
 * context stays shared and the test surface gets polluted.
 *
 * The Playwright suite uses headless Chromium and separate browser
 * contexts per role, giving us proper navigation, isolated sessions,
 * and auto-waiting assertions.
 *
 * This file is kept for reference only. It WILL be removed after the
 * Playwright suite proves itself stable in CI.
 */
...
vi.mock("@/lib/session/session-provider", async () => {
  const actual = await vi.importActual<
    typeof import("@/lib/session/session-provider")
...
function createJourneyBackend(options?: { expireReferenceList?: boolean }) {
  ...
  const idempotencyResponses = new Map<
    string,
    { fingerprint: string; response: ReferenceSummary }
  >();
...
describe("frontend runtime journey", () => {
  ...
  it("covers login, create, list, detail, and supervisor cancel with rendered screens", async () => {
    const fetchMock = vi
      .spyOn(global, "fetch")
      .mockImplementation(createJourneyBackend());
    ...
    expect(
      (await screen.findAllByText("Matrícula agosto")).length,
    ).toBeGreaterThan(0);
    ...
    searchParams = new URLSearchParams(
      "returnTo=%2Freferences%3Fcreated%3Dref-1",
    );

    render(<ReferenceDetailView referenceId="ref-1" />);

    expect(
      await screen.findByRole("heading", { name: "Matrícula agosto" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Cancelar referencia" }),
    ).toBeInTheDocument();
  ...
  it("handles auth loss on a protected route by redirecting back to login with context", async () => {
    ...
    sessionMockValue = {
      ...
      handleSessionError: (error, returnTo) => {
        ...
        replaceMock(
          `/login?reason=expired&returnTo=${encodeURIComponent(returnTo ?? "/references")}`,
        );
        return true;
      },
    };
